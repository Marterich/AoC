...
def up(CP):
	if (CP == 1):
		return((CablePath1[-1][0] +1, CablePath1[-1][1]))
	elif(CP == 2):
		return((CablePath2[-1][0] +1, CablePath2[-1][1]))
...
def left(CP):
	if (CP == 1):
		return((CablePath1[-1][0], CablePath1[-1][1] -1))
	elif(CP == 2):
		return((CablePath2[-1][0], CablePath2[-1][1] -1))
...
for x in range(len(Cable1)):
	
	if(Cable1[x][0] == "U"):
		for x in range(int(Cable1[x][1::])):
			CablePath1.append(up(1))

	elif(Cable1[x][0] == "D"):
		for x in range(int(Cable1[x][1::])):
			CablePath1.append(down(1))

	elif(Cable1[x][0] == "L"):
		for x in range(int(Cable1[x][1::])):
			CablePath1.append(left(1))

	elif(Cable1[x][0] == "R"):
		for x in range(int(Cable1[x][1::])):
			CablePath1.append(right(1))



for x in range(len(Cable2)):
	
	if(Cable2[x][0] == "U"):
		for x in range(int(Cable2[x][1::])):
			CablePath2.append(up(2))

	elif(Cable2[x][0] == "D"):
		for x in range(int(Cable2[x][1::])):
			CablePath2.append(down(2))

	elif(Cable2[x][0] == "L"):
		for x in range(int(Cable2[x][1::])):
			CablePath2.append(left(2))

	elif(Cable2[x][0] == "R"):
		for x in range(int(Cable2[x][1::])):
			CablePath2.append(right(2))


out = (set(CablePath1) & set(CablePath2))

sortedlist = []

for x in out:
	if (x != (0,0)):
		sortedlist.append((abs(x[0]), abs(x[1])))
...
for x in sorted(out):
	...
	for y in range(len(CablePath1)):
		if CablePath1[y] == x:
			temp = y

	for y in range(len(CablePath2)):
		if CablePath2[y] == x:

			temp += y

	if (temp != 0):
		steplist.append(temp)
	
print((sorted(steplist))[0])		
