...
def part2():
	...
	#iterate over Y
	while True:
		#start looking from the x position of the leftmost part of the beam in the previous row
		x = x_temp
		#Iterate over X
		while True:
			#check for the leftmost corner of the beam
			if computer(y,x) == 1:
				#save the leftmost corner as the new x starting position for the next line
				x_temp = x
				#check if the beam is at least 100 blocks wide
				if computer(y,x+99) == 1:
					#check if the beam is at least 100 blocks in height
					if computer(y+99, x) == 1:
						return(x*10000+y)
						
						# print("Santas Ship fits in at Y: {0} X: {1}".format(y,x))
						# print("The Result is:", str(x*10000+y))
